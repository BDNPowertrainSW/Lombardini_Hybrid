function msg = Lombardini_dual_fuel_can_example()
%% MOTOHAWK_CAN_EXAMPLE    MotoHawk CAN message definition example for sending or receiving single frames.
%%
...
%% the message payload.
%%
%% For an example of how to set up a CAN FD message, please see motohawk_can_fd_example.m
%%
...
%% .idcontent{}    - Allows control of the bit fields within message ID.    (optional)
%%                   For receive messages, this will create output ports on 
%%                   CAN read block that will provide portions of the message ID
%%                   to be used by the application. 
%%                   For send messages, this will allow runtime control of a portion
%%                   of the CAN message ID via input ports on the CAN send block.
%%                   If configuring a script for multi frame transmission, this field
%%                   allows the CAN ID to vary between the frames based on the values 
%%                   entered into the Node ID ports.   
%%
...
%%                   filter requiring the bits set in the payload mask to
...
%%                   Fields with data_type UNSIGNED or SIGNED cannot 
%%                   exceed 32 bits.
...
%%
  ...
  msg.name                    = 'ExampleMessage';
  msg.description             = 'Example message used as template in MotoHawk models';
  msg.protocol                = 'ExampleProtocol';
  msg.module                  = 'PCM-1';
  msg.bus_name                = 'CAN_1';

  msg.idext                   = 'STANDARD';
  msg.id                      = hex2dec('1f0');
  msg.idmask                  = hex2dec('fffffff0');
  msg.idinherit               =  0;
  msg.payload_size            =  8;
  msg.payload_value           = [];
  msg.payload_mask            = [];
  ...
  id_ctr = id_ctr + 1;
  msg.idcontent{id_ctr}.name       = 'Node ID';
  msg.idcontent{id_ctr}.start_bit  = 0;
  msg.idcontent{id_ctr}.bit_length = 4;

  field_ctr = 0;

  %% This field does NOT require all fields because it is a boolean (single bit)
  field_ctr = field_ctr + 1;
  msg.fields{field_ctr}.name          = 'Enable';
  msg.fields{field_ctr}.units         = '-';
  msg.fields{field_ctr}.start_bit     = 63;
  msg.fields{field_ctr}.bit_length    = 1;
  %%msg.fields{field_ctr}.byte_order    = 'BIG_ENDIAN'; %% Defaults to BIG_ENDIAN
  %%msg.fields{field_ctr}.data_type     = 'UNSIGNED';   %% Defaults to UNSIGNED
  %%msg.fields{field_ctr}.scale         = 1; %% Defaults to 1
  %%msg.fields{field_ctr}.offset        = 0; %% Defaults to 0
  %%msg.fields{field_ctr}.minimum       = 0; %% Defaults to minimum possible value in field
  %%msg.fields{field_ctr}.maximum       = 1;  %% Defaults to maximum possible value in field

  field_ctr = field_ctr + 1;
  msg.fields{field_ctr}.name          = 'Average Radius';
  msg.fields{field_ctr}.units         = 'm';
  msg.fields{field_ctr}.start_bit     = 44;
  msg.fields{field_ctr}.bit_length    = 12;
  msg.fields{field_ctr}.byte_order    = 'BIG_ENDIAN';
  msg.fields{field_ctr}.data_type     = 'SIGNED';
  msg.fields{field_ctr}.scale         = 1/10;
  msg.fields{field_ctr}.offset        = 0;
  %%msg.fields{field_ctr}.minimum       = -(2^11); %% Defaults to minimum possible value in field
  %%msg.fields{field_ctr}.maximum       = 2^11-1;  %% Defaults to maximum possible value in field

  field_ctr = field_ctr + 1;
  msg.fields{field_ctr}.name          = 'Temperature';
  msg.fields{field_ctr}.units         = 'degK';
  msg.fields{field_ctr}.start_bit     = 32;
  msg.fields{field_ctr}.bit_length    = 12;
  msg.fields{field_ctr}.byte_order    = 'BIG_ENDIAN';
  msg.fields{field_ctr}.data_type     = 'SIGNED';
  msg.fields{field_ctr}.scale         = 1/100;
  msg.fields{field_ctr}.offset        = 250.0;
  msg.fields{field_ctr}.minimum       = 229.53; %% In engineering units
  msg.fields{field_ctr}.maximum       = 270.47; %% In engineering units
